...
public class ScoreBoardWindow {
    ...
    public ScoreBoardWindow(int currentPage, String name, long time){
        ...
        table.getColumns().addAll(nicknameCol, scoreCol, modeCol, levelCol, linesCountCol, dateCol);
        table.setItems(data);
        table.setRowFactory(tv -> new TableRow<ScoreRecord>() {
            @Override
            protected void updateItem(ScoreRecord item, boolean empty) {
                super.updateItem(item, empty);
                if (item != null && !empty) {
                    // 이름과 타임스탬프가 생성자 매개변수와 일치하면 빨간색으로 표시
                    if (item.getNickname().equals(name) && item.getTimeStamp() == time) {
